...
func ExampleDisplayCycle() {
	...
	// (*(*a).Tail).Value = 0
	// (*(*(*a).Tail).Tail).Value = 0
	// (*(*(*(*a).Tail).Tail).Tail).Value = 0
	// (*(*(*(*(*a).Tail).Tail).Tail).Tail).Value = 0
	// (*(*(*(*(*(*a).Tail).Tail).Tail).Tail).Tail).Value = 0
	// (*(*(*(*(*(*(*a).Tail).Tail).Tail).Tail).Tail).Tail).Value = 0
	// (*(*(*(*(*(*(*(*a).Tail).Tail).Tail).Tail).Tail).Tail).Tail).Value = 0
	// (*(*(*(*(*(*(*(*(*a).Tail).Tail).Tail).Tail).Tail).Tail).Tail).Tail).Value = 0
	// (*(*(*(*(*(*(*(*(*(*a).Tail).Tail).Tail).Tail).Tail).Tail).Tail).Tail).Tail).Value = 0
}

func ExampleDisplayStructKeyedMap() {
	type TestStruct struct {
		Name  string
		Value int
	}
	m := map[TestStruct]int{
		TestStruct{"TestName", 100}: 1,
	}

	Display("m", m)
	// Output:
	// Display m (map[display.TestStruct]int):
	// m[display.TestStruct{.Name = "TestName", .Value = 100}] = 1
}

func ExampleDisplayArrayKeyedMap() {

	m := map[[5]int]string{
		[5]int{1, 2, 3, 4, 5}: "test",
	}

	Display("m", m)
	// Output:
	// Display m (map[[5]int]string):
	// m[[5]int{1, 2, 3, 4, 5}] = "test"
}
