...
func TestApploximatelyEqual(t *testing.T) {
	tests := []struct {
		x, y     interface{}
		expected bool
	}{
		...
		{complex(1.0, 1.0), complex(1.0, 1+1e-10), true},
		{complex(1.0, 1.0), complex(1.0, 1+1e-9), false},
	}
	for _, test := range tests {
		got := Equal(test.x, test.y)
		if got != test.expected {
			t.Errorf("Equal(%v, %v) = %t", test.x, test.y, !test.expected)
		}
	}
}
